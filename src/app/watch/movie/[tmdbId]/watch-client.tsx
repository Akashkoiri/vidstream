...
const PROVIDERS = [
  {
    ...
    url: (id: string, start: number) => {
      const url = new URL(`https://vidsrc2.ru/embed/movie/${id}`);
  ...
  {
    name: "CineSrc",
    url: (id: string) => `https://cinesrc.st/embed/movie/${id}`,
  },
  {
    name: "VidLink",
  ...
  {
    name: "Filmu",
    url: (id: string) => `https://embed.filmu.in/movie/${id}`,
  },
];
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "FILMU_PLAYER_EVENT") {
        const { event: playerEvent, currentTime, duration, mediaType, season, episode } =
          event.data.data;
